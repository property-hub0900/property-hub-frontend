...
export function RBACExample() {
  ...
  const groupPermissionsByCategory = (permissions: string[]) => {
    const categories = {
      ...
      favorites: permissions.filter(
        (p) => p.includes("favorites") || p.includes("saved")
      ),
    };
    ...
    <div className="w-full max-w-5xl mx-auto p-4 rounded-lg">
      ...
      <div className="mb-8 border-2 border-gray-200 rounded-lg p-4">
        <h2 className="text-lg font-semibold mb-3 text-center">
          Example UI for {selectedRole.toUpperCase()}
        </h2>
            ...
            <div className="text-xs text-gray-500 italic">
              {!hasPermission(PERMISSIONS.EDIT_PROPERTY) && (
                <div className="text-amber-600 flex items-center">
                  <Lock className="h-3 w-3 mr-1" /> Edit button hidden (no
                  permission)
                </div>
              )}
              {!hasPermission(PERMISSIONS.DELETE_PROPERTY) && (
                <div className="text-amber-600 flex items-center">
                  <Lock className="h-3 w-3 mr-1" /> Delete button hidden (no
                  permission)
                </div>
              )}
            </div>
          ...
          <div className="border rounded-md p-3">
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                disabled={!hasPermission(PERMISSIONS.ADMIN_DASHBOARD)}
              >
                ...
                {Object.entries(groupedPermissions).map(
                  ([category, perms]) =>
                    perms.length > 0 && (
                      <div
                        key={category}
                        className="border border-purple-200 rounded-md p-2 bg-white"
                      >
                        <h3 className="font-medium text-purple-700 mb-1 capitalize">
                          {category}
                        </h3>
            ...
            <div className="w-full max-w-3xl border-2 border-amber-500 rounded-lg p-3 bg-amber-50">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                {getAccessibleRoutes(selectedRole).map((route) => (
                  <div
                    key={route}
                    className="border border-amber-200 rounded-md p-2 bg-white flex items-center"
                  >
