...
export function StaffTable({
    ...
    const columns = [
        ...
        },
        {
            accessorKey: "role",
            header: "Type",
            cell: ({ row }: { row: any }) => <span className="capitalize">{row.original.role}</span>,
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }: { row: any }) => (
                <div className="flex space-x-2">
                    <Button variant="ghost" size="icon" onClick={() => onEdit(row.original)} aria-label="Edit staff member">
                        <Pencil className="h-4 w-4 text-blue-500" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => onDelete(row.original)} aria-label="Delete staff member">
