...
export function VerificationForm({ email, redirectPath, t }: VerificationFormProps) {
    ...
    const isFormDisabled = verificationStatus === "loading" || verificationStatus === "success"
    const canResend = resendTimer === 0 && verificationStatus !== "loading" && verificationStatus !== "success"
    ...
    const handleResendOtp = async () => {
        if (resendTimer > 0 || verificationStatus === "loading" || verificationStatus === "success") return
    ...
    return (
        ...
        >
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                        ...
                        >
                            <input
                                ...
                                className={cn(
                                    ...
                                    verificationStatus === "success" && "border-green-500 text-green-600",
                                    verificationStatus === "error" && "border-red-500 text-red-600",
                                )}
                ...
                <Button
                    type="submit"
                    className={cn("w-full relative", verificationStatus === "success" && "bg-green-600 hover:bg-green-700")}
                    disabled={verifyOtpMutation.isPending || !isOtpComplete || verificationStatus === "success"}
                    aria-busy={verifyOtpMutation.isPending}
                >
                    {verifyOtpMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin absolute left-4" />}
            ...
            <div className="text-center text-sm text-muted-foreground mt-6">{t("otpDisclaimer")}</div>
        </motion.div>
    )
}
